...
- **Correlate mini-dashboard popup** (nico-ops specific) — PRD-007 Slice 0 (#373). A centered Popup the operator opens with `[c]` on a focused row that mentions an entity ID; runs `nico_correlate::collect_all` in-process against the shared ops bootstrap (no second kubeconfig load, no second port-forward) and renders Diagnosis banner above a chronologically-sorted Event Timeline. State lives on `App::correlate` as `CorrelateState { entity: EntityRef, status: CorrelateStatus, diagnosis: Option<PopoverDiagnosis> }`. Two reducer entry points: `Action::Correlate` (translator-emitted on `[c]`) extracts the first entity from the focused finding via `extract_entity_from_finding` and re-dispatches as `Action::OpenCorrelatePopup(EntityRef)`; the latter is the general path the per-surface triggers (Spotlight today; logs / Findings detail / event timeline in later slices) all funnel into. Spotlight surfaces a "no entity found in this row" toast when extraction returns nothing; Scorecard preserves the issue-#157 silent-inert contract for the workflow-only legacy path. Slice 0 ships the basic body (Diagnosis + timeline); Slice 2 (#374) layers source-availability dots, streaming, and cancellation on top.
